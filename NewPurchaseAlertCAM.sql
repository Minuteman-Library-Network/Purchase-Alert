...
SELECT 
	...
	COUNT(DISTINCT h.id) FILTER(WHERE h.pickup_location_code = 'camz') AS local_holds,
	COUNT(DISTINCT i.id) AS item_count,
	COUNT(DISTINCT ia.id) AS avail_item_count, 
	COUNT(DISTINCT ia.id) FILTER(WHERE ia.location_code LIKE 'cam%' AND rmia.creation_date_gmt::DATE >= CURRENT_DATE - INTERVAL '14 days') AS in_process_item_count,
	COUNT(DISTINCT ia.id) FILTER(WHERE ia.location_code LIKE 'cam%') AS local_avail_item_count,
...
LEFT JOIN sierra_view.item_record ia
	...
	AND ((ia.location_code NOT LIKE 'cam%' AND ia.itype_code_num NOT IN ('5','21','109','133','160','183','239','240','241','244','248','249')) OR ia.location_code LIKE 'cam%')
...
LEFT JOIN (
   ...
   JOIN sierra_view.order_record_cmf oc
      ON oc.order_record_id=o.id AND oc.location_code LIKE 'cam%'
